...
def row_max_normalize(mat):
    mat = mat.tocsr()
    row_max = mat.max(axis=1).toarray().flatten()
    row_max[row_max == 0] = 1  # avoid division by zero
    row_indices, col_indices = mat.nonzero()
    data = mat.data / row_max[row_indices]
...
def main():
    ...
    for fname in os.listdir(consistency_dir):
        ...
        # --- WGCNA-style diagnostics ---
        mat_norm = row_max_normalize(mat)
        betas = list(range(1, 11))
        diag_png = os.path.join(mcl_dir, f'{species}.wgcna_diag.max.png')
        r2s = []
        mean_conns = []
        for beta in betas:
