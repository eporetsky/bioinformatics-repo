...
hisat2 -p 32 --max-intronlen 6000 -x ${hstref}.fa -U single.fastq.gz |				          
sambamba view -S -f bam -o /dev/stdout /dev/stdin |
sambamba sort  --tmpdir="tmpmba" -t 32 -o mapped/${sample}.bam /dev/stdin

rm single.fastq.gz

featureCounts -p -t exon,CDS -T 32 -a ${gtfref} -o counts/${sample}.counts mapped/${sample}.bam
